...
for fold, (tr_idx, va_idx) in enumerate(skf.split(train_df["ID"], train_df["target"])):
    ...
    scheduler = get_cosine_schedule_with_warmup(
        optimizer,
        num_warmup_steps=int(num_train_steps*0.03),
    ...
    for epoch in range(EPOCHS):
        train_ret = train_one_epoch(trn_loader, model, optimizer, loss_fn, device=device, scheduler=scheduler)
...
pred_df.to_csv(f"{OUTPUT_PATH}/output_maxvit.csv", index=False)
np.save(f"{OUTPUT_PATH}/probs_maxvit.npy", test_probs_avg)
