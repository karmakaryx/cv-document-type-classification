...
EARLY_STOPPING_PATIENCE = 3     # val_f1이 개선되지 않으면 중단할 epoch 수
...
### 3. Load Data ###
augraphy_pipeline = augraphy.AugraphyPipeline([
...
for fold, (tr_idx, va_idx) in enumerate(skf.split(train_df['ID'], train_df['target'])):
    ...
    # 분할된 train 데이터 내에서만 클래스 증식
    target_counts = {
        7: 500, 3: 450, 4: 400, 14: 400,
        6: 200, 12: 250, 13: 250,
        10: 200, 11: 200, 15: 200,
        1: 150, 0: 100,
    }
    ...
    trn_dataset = ConcatDataset([base_dataset, aug_dataset])
    ...
    for epoch in range(EPOCHS):
        train_ret = train_one_epoch(trn_loader, model, optimizer, loss_fn, device=device, scheduler=None)
        scheduler.step()
    ...
    # Snapshot Ensemble 추론
    ckpt_files = glob.glob(os.path.join(CHECKPOINT_DIR, f'best_fold{fold}_epoch*.pt'))
    ckpt_files = sorted(ckpt_files, key=lambda x: os.path.getmtime(x), reverse=True)[:3]  #최신 3개
    print(f'\n[Fold {fold}] Snapshot Ensembling with: {ckpt_files}')

    fold_probs_list = []
    for ckpt in ckpt_files:
        load_checkpoint(ckpt, model, device)  #모델에 가중치 덮어쓰기
        probs = predict_proba(tst_loader, model, device)  #TTA 포함된 확률 추출
        fold_probs_list.append(probs)

    fold_test_probs = np.mean(fold_probs_list, axis=0)
...
pred_df.to_csv(f'{DATA_PATH}/output_conv.csv', index=False)
np.save(f'{DATA_PATH}/probs_conv.npy', test_probs_avg)
