...
if __name__ == "__main__":
    ...
    for fold in range(N_SPLITS):
        ...
        if not ckpt_files:
            print(f"fold {fold}에 해당하는 파일이 없습니다!")
            continue
    ...
    output_name = "submission_conv_recovery.csv"
    test_df.to_csv(os.path.join(OUTPUT_PATH, output_name), index=False)
    np.save(os.path.join(OUTPUT_PATH, "probs_conv_recovery.npy"), final_probs)
