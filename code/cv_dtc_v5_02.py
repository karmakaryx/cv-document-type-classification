...
trn_transform = A.Compose([
    ...
    ], p=1.0),  # 50:50 비율로 꽉찬 이미지와 잘린 이미지를 섞어서 학습

    # 미세 변형
    A.ShiftScaleRotate(rotate_limit=20, scale_limit=0.1, p=0.5, border_mode=0, value=(0,0,0)),
    ...
    A.OneOf([
        ...
        A.MotionBlur(blur_limit=7, p=1.0),  # 흔들린 사진
        A.GaussNoise(var_limit=(100.0, 300.0), p=1.0),  # 강력한 모래알 노이즈
        A.ImageCompression(quality_lower=30, quality_upper=60, p=1.0),  # 화질 저하 극대화
        A.ISONoise(color_shift=(0.01, 0.05), intensity=(0.1, 0.5), p=1.0),
        A.CoarseDropout(max_holes=8, max_height=IMG_SIZE//10, max_width=IMG_SIZE//10, p=1.0),
